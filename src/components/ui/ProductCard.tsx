...
const ProductCard = ({ 
  ...
  const imagePath = image.startsWith('/') || image.startsWith('http') 
    ? image
  ...
  return (
    ...
    >
      <Link to={`/product/${id}`} className="block group h-full">
        <div className="relative bg-white rounded-lg overflow-hidden border border-gray-100 transition-all duration-300 hover:shadow-lg h-full">
          {/* Badge */}
          {bestseller && (
            <motion.div 
              ...
              className="absolute top-4 left-4 z-10 bg-luxe-gold text-white text-xs py-1 px-2 rounded"
            >
              Bestseller
            </motion.div>
          )}
          ...
          <div className="relative h-64 overflow-hidden">
            ...
            {/* Quick add button - appears on hover with animation */}
            <motion.button 
          ...
          <div className="p-4 flex flex-col h-[calc(100%-16rem)]">
            ...
            {/* Rating */}
            <div className="mt-3 flex items-center mt-auto">
              <div className="flex">
                {[...Array(5)].map((_, i) => (
                  <svg 
                    key={i} 
